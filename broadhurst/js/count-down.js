...
var hour=07;      //-->Enter the count down target date HOUR (24 hour clock)
...
function countdown(yr,m,d,hr,min,mil){
    theyear=yr;themonth=m;theday=d;thehour=hr;theminute=min;themilli=mil;
    var today=new Date();
    ...
    var todaymilli=today.getMilliseconds();
    var todaystring1=montharray[todaym]+" "+todayd+", "+todayy+" "+todayh+":"+todaymin+":"+todaysec+" "+todaymilli;
    var todaystring=Date.parse(todaystring1)+(tz*1000*60*60);
  var futurestring1=(montharray[m-1]+" "+d+", "+yr+" "+hr+":"+min+" "+mil);
  ...
  var dmilli=Math.floor((todaymilli / 10) % 100);
    if(dday<=0&&dhour<=0&&dmin<=0&&dsec<=0&&dmilli<0){
      ...
      document.getElementById('dmilli').innerHTML=dmilli;
        setTimeout("countdown(theyear,themonth,theday,thehour,theminute,themilli)",1);
    }
}
